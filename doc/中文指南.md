...
✅ **已完成功能**:
- RSS 源并发抓取（94个学术期刊源）
- Gmail 邮件解析（支持 Google Scholar、期刊 TOC 邮件）
- 两级过滤系统（关键词 OR + AI 语义精选）
- CrossRef/OpenAlex 元数据补充
- JSON/Zotero 导出
- 完整 CLI 工具链
- 350+ 单元测试（100% 通过）

📋 **当前配置**:
- 支持 Nature、Science、ACS、Wiley、RSC、Elsevier、IOP 等主流期刊
- AI 过滤使用 DeepSeek/OpenAI 兼容 API
- 所有敏感配置通过 `.env` 管理

### 更新历史

**v1.2.0 (2026-02-08)**
- ✅ PaperItem 扩展至完整 Zotero journalArticle 字段（publication_title, publisher, volume, issue, pages, issn, language 等 20+ 字段）
- ✅ `metadata` 字段重命名为 `extra`（语义更清晰，避免与 Pydantic 内部冲突）
- ✅ RSS 去重升级为三级优先级：DOI（大小写不敏感）> URL > 标题（模糊匹配，去除 ASAP 等噪声词）
- ✅ CrossRef 富化策略变更为**覆盖**所有字段（最权威来源）
- ✅ OpenAlex 富化策略保持**仅补充缺失**字段
- ✅ 两个 API 的未映射字段存入 `extra["crossref_extra"]` / `extra["openalex_extra"]`
- ✅ CLI enrich 命令：OpenAlex 始终运行（不再因 CrossRef 已补充而跳过）
- ✅ Zotero 适配器映射所有新增字段
...
- ✅ 所有 CLI 命令测试通过
- ✅ 文档完善（中文指南 + README）
- ✅ 关键词过滤逻辑确认为 OR（而非 AND）
- ✅ AI 过滤模块从 `ai/` 移动到 `filters/` 以更符合逻辑分层
- ✅ 清理所有缓存和临时文件
- ✅ .gitignore 配置完善
...
cp .env.example .env
# 编辑 .env 文件，至少配置：
# - OPENAI_API_KEY（用于 AI 过滤）
# - POLITE_POOL_EMAIL（用于元数据补充）
...
```bash
# 步骤 1: 获取 20 篇最新论文
uv run paper-feed fetch --source rss --limit 20 --output papers.json

# 步骤 2: 用关键词过滤（替换为你的研究领域关键词）
uv run paper-feed filter --input papers.json --output filtered.json \
    --keywords battery energy chemistry

# 步骤 3: 查看结果
cat filtered.json | python -m json.tool | head -30

# 步骤 4: 导出（可选）
uv run paper-feed export --input filtered.json --output final.json \
    --format json --include-metadata
```
...
```bash
# 1. 获取论文（从 94 个学术期刊源）
uv run paper-feed fetch --source rss --limit 100 --output raw.json

# 2. 第一级过滤：关键词（宽筛）
uv run paper-feed filter --input raw.json --output keyword_filtered.json \
    --keywords battery electrode energy storage

# 3. 第二级过滤：AI 语义（精选）- 需配置 OPENAI_API_KEY 和 RESEARCH_PROMPT
uv run paper-feed filter --input keyword_filtered.json --output ai_filtered.json --ai

# 4. 元数据补充（DOI、作者、摘要等）
uv run paper-feed enrich --input ai_filtered.json --output enriched.json \
    --source all --concurrency 5

# 5. 最终导出
uv run paper-feed export --input enriched.json --output final_papers.json \
    --format json --include-metadata
```
...
│   │
│   ├── ai/                    # AI 模块
│   │   ├── __init__.py        # 导出: KeywordGenerator
│   │   └── keyword_generator.py # OpenAI 驱动的关键词生成
│   │
│   ├── filters/               # 过滤管道
│   │   ├── __init__.py        # 导出: FilterPipeline, KeywordFilterStage, AIFilterStage
│   │   ├── pipeline.py        # FilterPipeline — 顺序阶段编排器
│   │   ├── keyword.py         # KeywordFilterStage — OR/NOT 关键词逻辑
│   │   └── ai_filter.py       # AIFilterStage — LLM 语义论文过滤
│   │
│   ├── adapters/              # 导出适配器
...
```
CLI (fetch --source rss) → RSSSource
    ├─ OPMLParser.parse() → List[{url, title, category}]
    ├─ httpx.AsyncClient.get(url)  ×  asyncio.Semaphore(max_concurrent)
    ├─ asyncio.to_thread(feedparser.parse, content) → feed
    ├─ RSSParser.parse(entry, source_name, feed_meta) → PaperItem
    └─ 去重（url or title） → List[PaperItem]

CLI (fetch --source gmail) → GmailSource
    ├─ _write_json_configs()  →  feeds/token.json + feeds/credentials.json（仅在内联 JSON 模式）
    ├─ ezgmail.init() → ezgmail.search(query, maxResults)
    ├─ _extract_html_body(message.messageObj)  →  HTML
    ├─ GmailParser.parse(html, source_name) → List[PaperItem]
    ├─ _extract_from_plain_text()（HTML 不可用时的 DOI 正则回退）
    └─ _deduplicate(title) → List[PaperItem]

CLI (filter) → FilterPipeline(llm_client?)
    ├─ Stage 1: KeywordFilterStage
    │   ├─ exclude_keywords → OR 排除
    │   ├─ keywords → OR 匹配（一级宽筛选）
    │   ├─ authors → OR 匹配
    │   ├─ has_pdf → pdf_url 检查
    │   └─ min_date → published_date 检查
    ├─ Stage 2: AIFilterStage（可选，需 llm_client）
    │   ├─ 分批（50 篇/批）→ 中文 prompt → LLM API
    │   └─ 解析 JSON {"relevant": [...]} → 索引验证
    └─ FilterResult { papers, total_count, passed_count, rejected_count, filter_stats }

CLI (enrich --source all) → CrossrefClient + OpenAlexClient
    ├─ 每篇论文（asyncio.Semaphore(concurrency)）:
    │   ├─ CrossRef: get_by_doi() → search_by_title() → enrich_paper()
    │   ├─ 如果 CrossRef 已补充 → 跳过 OpenAlex
    │   └─ 否则 OpenAlex: get_by_doi() → search_by_title() → enrich_paper()
    └─ 只补充缺失字段，额外元数据 → metadata["crossref"] / metadata["openalex"]

CLI (export) → JSONAdapter / ZoteroAdapter
    ├─ JSONAdapter.export() → JSON 文件（model_dump, date→ISO, indent=2）
    └─ ZoteroAdapter.export() → Zotero API（journalArticle 格式）
```
...
通用论文表示，对齐 Zotero journalArticle schema。所有数据源产出此对象；所有适配器消费此对象。

```python
class PaperItem(BaseModel):
    # --- 核心字段（必填） ---
    title: str                                          # 论文标题
    source: str                                         # 来源名称（如 "Nature", "ACS"）
    source_type: str                                    # "rss" 或 "email"

    # --- 文献信息字段（可选） ---
    authors: List[str] = Field(default_factory=list)    # 作者姓名列表
    abstract: str = Field(default="")                   # 摘要/概要
    published_date: Optional[date] = None               # 发表日期
    doi: Optional[str] = None                           # DOI（无前缀）
    url: Optional[str] = None                           # 论文 URL
    pdf_url: Optional[str] = None                       # PDF 直链
    publication_title: Optional[str] = None             # 期刊名
    journal_abbreviation: Optional[str] = None          # 期刊缩写
    publisher: Optional[str] = None                     # 出版商
    place: Optional[str] = None                         # 出版地
    volume: Optional[str] = None                        # 卷号
    issue: Optional[str] = None                         # 期号
    pages: Optional[str] = None                         # 页码范围（如 "123-145"）
    section: Optional[str] = None                       # 文章分区
    part_number: Optional[str] = None                   # 部分编号
    part_title: Optional[str] = None                    # 部分标题
    series: Optional[str] = None                        # 系列名称
    series_title: Optional[str] = None                  # 系列标题
    series_text: Optional[str] = None                   # 系列文本
    citation_key: Optional[str] = None                  # 引用键
    access_date: Optional[date] = None                  # 访问日期
    pmid: Optional[str] = None                          # PubMed ID
    pmcid: Optional[str] = None                         # PubMed Central ID
    issn: Optional[str] = None                          # ISSN
    archive: Optional[str] = None                       # 档案名
    archive_location: Optional[str] = None              # 档案位置
    short_title: Optional[str] = None                   # 简短标题
    language: Optional[str] = None                      # 语言
    library_catalog: Optional[str] = None               # 图书馆目录来源
    call_number: Optional[str] = None                   # 索书号
    rights: Optional[str] = None                        # 版权/许可
    item_type: str = "journalArticle"                   # 条目类型

    # --- 内部字段 ---
    source_id: Optional[str] = None                     # 来源特定 ID
    extra: Dict[str, Any] = Field(default_factory=dict) # 扩展数据（富化元数据、feed 信息等）
```

**约束：**
- `title`、`source` 和 `source_type` 始终必填
- `doi` 可选（`None`），不是空字符串 — enrichment 阶段可能补充
- `extra` 是扩展数据字段 — 用于存储 API 富化的额外元数据（如 `extra["crossref"]`、`extra["openalex"]`、`extra["feed"]`）
- 文献信息字段（volume, issue, pages 等）由 CrossRef/OpenAlex 直接映射，不再存放于 `extra` 中
- 所有可变类型的默认值使用 `Field(default_factory=...)`，不使用 `= []` 或 `= {}`
...
**重要**: `keywords` 字段使用 **OR 逻辑**，而非 AND 逻辑。这确保了第一级过滤不会过于严格，避免误杀潜在相关论文。
...
- **构造函数**: `RSSSource(opml_path?, user_agent?, timeout?, max_concurrent?)`
  - 所有参数可选，回退到 `get_rss_config()` → 环境变量 → 硬编码默认值
- **依赖**: `feedparser`, `httpx`（异步 HTTP）, `OPMLParser`, `RSSParser`
- **并发控制**: `asyncio.Semaphore(max_concurrent)` 控制并行抓取（默认 10）
- **去重**: 按 `paper.url or paper.title` 跨所有订阅源去重（URL 优先，无 URL 时用标题）
- **错误处理**: 失败的订阅源记录日志并跳过；`asyncio.gather(return_exceptions=True)`
- **来源名称检测**: `_detect_source_name(url)` — URL 模式匹配已知出版商域名（nature.com→"Nature"、acs.org→"ACS" 等），未命中时从域名首段推断
- **全局 limit**: 聚合后截断，`all_papers[:limit]`
...
- **构造函数**: `GmailSource(query?, source_name?, max_results?, mark_as_read?, auto_detect_source?, processed_label?, trash_after_process?)`
  - 允许两种凭据方式：
    - 已有文件：`feeds/token.json` + `feeds/credentials.json`（默认）
    - 内联 JSON：`GMAIL_TOKEN_JSON` / `GMAIL_CREDENTIALS_JSON`（会写入上述文件路径）
  - 若文件与内联 JSON 都不存在，则抛 `ValueError`
- **依赖**: `ezgmail`, `GmailParser`
- **延迟初始化**: `_ensure_init()` → `_write_json_configs()`（若提供内联 JSON 则写入文件）→ `ezgmail.init(tokenFile, credentialsFile)`
...
- **纯文本回退**: `_extract_from_plain_text()` — 当无 HTML 时，从 `originalBody` / `body` 中用 `DOI_PATTERN` 正则提取 DOI 链接
- **发件人自动检测**: `_detect_source_from_sender()` — 通过 `_SENDER_SOURCE_MAP` 映射（如 `scholaralerts-noreply@google.com` → "Google Scholar"）自动设置来源名称
- **附件发现**: `_extract_attachment_info()` — 从 `message._attachmentsInfo` / `message.attachments` 中提取 PDF 文件名（当前不写入输出）
- **去重**: `_deduplicate()` — 按标题（不区分大小写）跨所有邮件去重
- **功能**: 日期过滤、标记已读（`thread.markAsRead()`）、处理标签（`message.addLabel()`）、失败标记未读（`thread.markAsUnread()`）、处理成功后移入回收站（`thread.trash()`，默认开启）
  - 处理完成后默认执行 Trash 核对查询（`in:trash`）
...
- 嵌套类别（递归 `<outline>` 遍历）
- 提取条件: `type="rss"` **且** `xmlUrl` 属性存在
- 返回 `List[Dict]`，每项包含 `url`、`title`、`html_url`、`category`
- 类别保留（从父级 outline 的 `text` 属性继承）

### 5.6 CrossrefClient (`sources/crossref.py`)

- **用途**: CrossRef REST API 客户端，用于 DOI 查找和论文元数据补充。
- **核心类**: `CrossrefWork`（dataclass）— 包含 doi, title, authors（"Family, Given" 格式）, journal, year, abstract, citation_count, pdf_url 等 17 个字段。
- **方法**:
  - `search_by_title(title, rows=5)`: 按标题搜索，使用 `select` 参数减少响应负载。
  - `get_by_doi(doi)`: 按 DOI 精确查找。
  - `find_best_match(title, threshold=0.8)`: Jaccard 相似度（词集交/并）匹配最佳结果。
  - `enrich_paper(paper) → PaperItem`: DOI 查找优先 → 标题搜索回退。**只补充缺失字段**（abstract, authors, doi, url, pdf_url, published_date）。额外元数据存入 `metadata["crossref"]`（journal, publisher, citation_count 等）。返回 `model_copy(update=...)` 新实例。
  - `close()`: 关闭 httpx AsyncClient。
- **Polite Pool**: 通过 `User-Agent` 和 `mailto` 头部获取更高速率限制。

### 5.7 OpenAlexClient (`sources/openalex.py`)

- **用途**: OpenAlex REST API 客户端，用于开放元数据补充。
- **核心类**: `OpenAlexWork`（dataclass）— 包含 doi, title, authors（display name）, journal, cited_by_count, concepts 等 14 个字段。
- **方法**:
  - `search_by_title(title, per_page=5)`: 按标题搜索。
  - `get_by_doi(doi)`: 按 DOI 精确查找。
  - `find_best_match(title, threshold=0.8)`: 同 CrossRef 的 Jaccard 相似度。
  - `enrich_paper(paper) → PaperItem`: 同 CrossRef 逻辑。额外元数据存入 `metadata["openalex"]`（journal, cited_by_count, concepts 等）。
  - `_reconstruct_abstract(inverted_index)`: 从 OpenAlex 倒排索引 `{word: [positions]}` 重建连续文本。
  - `close()`: 关闭 httpx AsyncClient。
...
顺序阶段编排器。构造函数: `FilterPipeline(llm_client=None)`

```python
# llm_client 为 None 时只有关键词阶段
pipeline = FilterPipeline()

# 传入 OpenAI client 时启用 AI 阶段（条件导入 AIFilterStage）
pipeline = FilterPipeline(llm_client=OpenAI(api_key=..., base_url=...))
```

两个阶段按顺序执行，每阶段通过 `is_applicable(criteria)` 判断是否运行：
- **Stage 1**: `KeywordFilterStage` — 当 keywords/exclude_keywords/authors/min_date/has_pdf 任一非默认值时运行
- **Stage 2**: `AIFilterStage` — 当 `llm_client` 已提供且 `_client is not None` 时运行。通过 `try/except ImportError` 条件导入，模块不可用时跳过并警告
...
LLM 驱动的语义相关性判断。构造函数: `AIFilterStage(openai_client=None, model=None)`

- **批处理**: 按批次处理论文（默认: 50 篇，`AI_BATCH_SIZE` 可配），每批一次 API 调用。
- **提示语言**: **中文**。System prompt: "你是一个学术论文相关性判断助手"。User prompt 包含 `RESEARCH_PROMPT` + 编号论文列表（摘要截断至 500 字符）。
- **LLM 参数**: `temperature=0.0`, `max_tokens` 从 `AI_FILTER_MAX_TOKENS` 读取（默认 1000）。
- **输出格式**: 期望 JSON `{"relevant": [0, 3, 7, ...]}`（从 0 开始的索引）。
- **鲁棒解析** (`_parse_filter_output`): 3 种策略依次尝试：
  1. 直接 `json.loads()` 解析
  2. 从 markdown 代码块 `` ```json {...}``` `` 中提取
  3. 正则搜索输出中任意位置的 `{"relevant": [...]}` 对象
- **索引验证**: `_validate_indices()` 过滤越界和非整数值。
- **失败开放**: API 调用异常时，**返回该批次所有论文**（视为全部相关），确保不因技术错误丢失论文。解析失败时返回空集（该批次全部排除）。
- **前置条件**: 无 `OPENAI_API_KEY` 或无 `RESEARCH_PROMPT` 时跳过，返回原始列表。

---

## 9. 配置系统 (`core/config.py`)
...
OPENAI_BASE_URL=https://api.deepseek.com/v1  # 可选，使用 DeepSeek
RESEARCH_PROMPT="我的研究兴趣是..."  # AI 过滤的判断依据

# Gmail 源（使用 --source gmail 时必需，二选一）
# 方式 A: 直接提供文件（默认路径）
# feeds/token.json
# feeds/credentials.json
# 方式 B: 内联 JSON（启动时写入 feeds/ 目录）
...
```bash
# 元数据补充优化
POLITE_POOL_EMAIL=your@email.com  # CrossRef/OpenAlex 高速访问

# RSS 源配置
PAPER_FEED_OPML=feeds/RSS_official.opml  # 默认 OPML 文件路径
RSS_MAX_CONCURRENT=10  # 并发抓取数量
```
...
# AI 过滤参数
AI_BATCH_SIZE=50  # 每批处理论文数
AI_FILTER_MAX_TOKENS=1000  # AI 过滤响应 token 限制

# Gmail 处理行为
GMAIL_TRASH_AFTER_PROCESS=true  # 处理成功后移入回收站
GMAIL_VERIFY_TRASH_AFTER_PROCESS=true  # 处理后执行 Trash 核对
GMAIL_VERIFY_TRASH_LIMIT=50  # Trash 核对最大线程数
...
支持两种方式（二选一）：
- **文件模式（推荐）**：将 `token.json` 与 `credentials.json` 放在 `feeds/` 目录
- **内联 JSON 模式**：设置 `GMAIL_TOKEN_JSON` 与 `GMAIL_CREDENTIALS_JSON`，程序启动时写入 `feeds/`

**获取步骤**:
1. 在 Google Cloud Console 创建 OAuth2 凭据
2. 下载 `credentials.json` 并放到 `feeds/credentials.json`
3. 运行 ezgmail 初始化生成 `token.json`，放到 `feeds/token.json`
4. 或将两个 JSON 内容写入 `.env`（内联 JSON 模式）

**注意**: 文件路径可通过 `GMAIL_TOKEN_FILE` / `GMAIL_CREDENTIALS_FILE` 覆盖，默认使用 `feeds/` 目录。

### 共享 API 配置

CrossRef 和 OpenAlex 共享以下默认值，避免重复配置：
- `POLITE_POOL_EMAIL` → 共享邮箱（强烈推荐配置）
- `API_TIMEOUT` → 共享超时
...
- **`enrich`**: 通过外部 API 进行元数据补充。
  ...
  - `--concurrency`: 最大并行 API 请求数（默认: 5，通过 `asyncio.Semaphore` 控制）。
  - **回退逻辑**（`--source all` 时）: CrossRef 先尝试 → 如果 CrossRef 成功补充了字段（doi/authors/date 有变化），跳过 OpenAlex；否则 OpenAlex 作为回退。每个 client 在 `finally` 中调用 `close()` 释放连接。
- **`export`**: 导出论文到目标格式。
...
# 完整 Pipeline 示例
# 1. 从 RSS 获取论文
uv run paper-feed fetch --source rss --limit 50 --output raw.json

# 2. 关键词过滤
uv run paper-feed filter --input raw.json --output filtered.json \
    --keywords battery energy lithium

# 3. AI 语义过滤（需配置 OPENAI_API_KEY）
uv run paper-feed filter --input filtered.json --output ai_filtered.json \
    --keywords battery --ai
...
# 5. 导出
uv run paper-feed export --input enriched.json --output final.json \
    --format json --include-metadata
```
...
- `export(papers, filepath="papers.json", include_metadata=True) → Dict`
- 将 `List[PaperItem]` 导出为 JSON 文件（`model_dump()` 序列化）
- 将 `date` 对象转换为 ISO 格式字符串
- `include_metadata=False` 时移除 `metadata` 字段
- 自动创建父目录（`Path.mkdir(parents=True)`）
- UTF-8 编码，美化输出（`indent=2, ensure_ascii=False`）
- 返回 `{"count": int, "filepath": str, "success": bool}`
...
**1. Pipeline 工作流程**
```bash
# 推荐：渐进式过滤，避免一次性处理太多论文
uv run paper-feed fetch --source rss --limit 100 --output step1_raw.json
uv run paper-feed filter --input step1_raw.json --output step2_keyword.json \
    --keywords "你的关键词"
uv run paper-feed filter --input step2_keyword.json --output step3_ai.json --ai
uv run paper-feed enrich --input step3_ai.json --output step4_enriched.json
uv run paper-feed export --input step4_enriched.json --output final.json \
    --include-metadata
```

**2. 关键词策略**
- **一级过滤**: 使用宽泛的关键词（3-5个），利用 OR 逻辑捕获尽可能多的相关论文
- **二级过滤**: 配置详细的 `RESEARCH_PROMPT`，让 AI 进行精确判断
- **示例**: 关键词用 `battery zinc lithium`，RESEARCH_PROMPT 详细描述研究兴趣

**3. API 配置优化**
- **CrossRef/OpenAlex**: 配置 `POLITE_POOL_EMAIL` 获得更高速率限制
- **并发控制**: enrichment 时使用 `--concurrency 3-5`，避免触发速率限制
- **AI 过滤**: 使用 DeepSeek API（更便宜）而非 OpenAI

**4. 数据管理**
- 保留中间 JSON 文件，便于调试和重新过滤
- 使用有意义的文件名（如 `2026-02-07_battery_papers.json`）
- 定期备份 `.env` 文件（但不要提交到 git）

### 常见问题

**Q1: 为什么 AI 过滤没有生效？**
- 检查 `OPENAI_API_KEY` 是否正确设置
- 检查 `RESEARCH_PROMPT` 是否已配置
- 查看日志中是否有 API 错误信息

**Q2: Gmail 源报错 "GMAIL_TOKEN_JSON not found"？**
- 需要先通过 Gmail OAuth2 获取 token 和 credentials
- 方式 A：放到 `feeds/token.json` 与 `feeds/credentials.json`
- 方式 B：将 JSON 内容作为字符串设置到 `.env`
- 参考 `.env.example` 中的格式

**Q3: Enrichment 速度很慢？**
- CrossRef/OpenAlex API 有速率限制
- 减少并发数（`--concurrency 3`）
- 只对需要补充的论文进行 enrichment

**Q4: 过滤结果太少或太多？**
- **太少**: 放宽关键词，检查是否使用了 `--exclude`
- **太多**: 添加更具体的关键词，启用 AI 过滤
...
**RSS 源抓取**:
- 调整 `RSS_MAX_CONCURRENT`（默认 10）控制并发数
- 使用 `--limit` 限制论文数量
- 使用 `--since` 只获取最近的论文

**过滤性能**:
- 关键词过滤非常快（~1000 篇/秒）
- AI 过滤较慢（受 API 速率限制），批量处理 50 篇/次
...
**1. `ModuleNotFoundError: No module named 'paper_feed'`**
```bash
# 解决：确保使用 uv run
uv sync  # 先同步依赖
uv run paper-feed --help
...
**3. `HTTPError: 429 Too Many Requests`（CrossRef/OpenAlex）**
```bash
# 解决：
# 1. 减少并发数
uv run paper-feed enrich --concurrency 2 ...
# 2. 配置 POLITE_POOL_EMAIL 获得更高限制
echo "POLITE_POOL_EMAIL=your@email.com" >> .env
```

**4. AI 过滤返回所有论文（失败开放）**
```bash
# 原因：API 调用失败，框架自动返回所有论文防止数据丢失
# 解决：
# 1. 检查 API key 是否正确
# 2. 检查网络连接
# 3. 查看详细日志：uv run paper-feed filter ... 2>&1 | tee log.txt
```

**5. `UnicodeDecodeError` 或乱码**
```bash
# 解决：确保所有文件使用 UTF-8 编码
# Windows 用户：在 .env 中添加
...
```bash
# 检查清单：
# 1. feeds/token.json 与 feeds/credentials.json 已存在，或
#    GMAIL_TOKEN_JSON 与 GMAIL_CREDENTIALS_JSON 已设置
# 2. JSON 格式正确（使用单引号包裹整个 JSON 字符串）
# 3. Gmail API 已在 Google Cloud Console 中启用
# 4. OAuth2 consent screen 已配置
```
...
uv run paper-feed fetch ...
```

**检查中间输出**:
```bash
# 查看每一步的输出文件
cat step1_raw.json | python -m json.tool | head -50
```
