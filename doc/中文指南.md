...
uv run paper-feedder-mcp filter --input output/keyword_filtered.json --output output/ai_filtered.json --ai
...
CLI (enrich --source all) → CrossrefClient + OpenAlexClient
    ...
    │   └─ OpenAlex: get_by_doi() → search_by_title() → enrich_paper()
...
- **方法**:
  ...
  - `enrich_paper(paper) → PaperItem`: DOI 查找优先 → 标题搜索回退。**只补充缺失字段**（abstract, authors, doi, url, pdf_url, published_date）。额外元数据存入 `extra["crossref"]`（journal, publisher, citation_count 等）。返回 `model_copy(update=...)` 新实例。
...
- **方法**:
  ...
  - `enrich_paper(paper) → PaperItem`: 同 CrossRef 逻辑。额外元数据存入 `extra["openalex"]`（journal, cited_by_count, concepts 等）。
...
- **`enrich`**: 通过外部 API 进行元数据补充。
  ...
  - **执行逻辑**（`--source all` 时）: CrossRef 与 OpenAlex 依次执行（均会尝试补充缺失字段）。每个 client 在 `finally` 中调用 `close()` 释放连接。
- **`export`**: 导出论文到目标格式。
  - `--format`: `json`（默认）或 `zotero`。
  - `--include-metadata`: 包含来源特定扩展字段（`extra`）。
...
- `export(papers, filepath="output/papers.json", include_metadata=True) → Dict`
...
- 批处理，每项错误单独处理
- **成功导出后**自动清理 `output/` 和常见中间文件（CLI `export --format zotero`）
...
uv run paper-feedder-mcp filter --input output/step1_raw.json --output output/step2_keyword.json \
    --keywords "你的关键词"
uv run paper-feedder-mcp filter --input output/step2_keyword.json --output output/step3_ai.json --ai
