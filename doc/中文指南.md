# paper-feedder-mcp 框架指南

> 本文档为 AI agent 及人类开发者提供 paper-feedder-mcp 代码库的全面说明：架构、规范、数据流与约束。

---

## 1. 项目概述

**paper-feedder-mcp** 是一个模块化 Python 框架，用于从 RSS 订阅源和 Gmail 邮件提醒中收集、过滤和导出学术论文。

- **语言**: Python 3.12+
- **异步优先**: 所有 I/O 操作使用 `async/await`
- **包管理器**: `uv`（锁文件: `uv.lock`），构建后端: `hatchling`
- **代码检查/格式化**: `ruff`（行宽 88，目标 py312）
- **类型检查**: `ty`（Red Knot）
...
# 3. 第二级过滤：AI 语义（精选，默认开启）- 需配置 OPENAI_API_KEY 和 RESEARCH_PROMPT
...
│   ├── client/                # CLI（默认 serve + 子命令）
│   │   └── cli.py
│   ├── config/                # 配置（pydantic-settings）
│   │   └── settings.py
│   ├── handlers/              # MCP tools / prompts
│   │   ├── tools.py
│   │   └── prompts.py
│   ├── models/                # 架构模型
│   │   ├── responses.py       # PaperItem 等核心实体
│   │   ├── schemas.py         # MCP 输入 schema
│   │   └── enums.py           # ToolName enum
│   ├── services/              # 业务服务层
│   │   ├── fetch.py
│   │   ├── filter.py
│   │   ├── enrich.py
...
└─────────────────────────────────────────────────┘
        ...
        ▼
┌─────────────────────────────────────────────────┐
│           Handlers (tools.py / prompts.py)      │
└─────────────────────────────────────────────────┘
        │
        ▼
┌─────────────────────────────────────────────────┐
│            Services (fetch/filter/enrich/export)│
└─────────────────────────────────────────────────┘
        │
   ┌────┼────────────┬────────────┐
   ▼    ▼            ▼            ▼
...
CLI (enrich --api all) → CrossrefClient + OpenAlexClient
    ├─ 每篇论文（asyncio.Semaphore(concurrency)）:
    │   ├─ CrossRef: DOI → 标题 → URL（优先 DOI；覆盖已有字段）
    │   └─ OpenAlex: DOI → 标题 → URL（只补充缺失字段）
    └─ 只补充缺失字段，额外元数据 → extra["crossref"] / extra["openalex"]
...
- **发件人自动检测**: `_detect_source_from_sender()` — 通过 `GMAIL_SENDER_MAP_JSON` 映射（如 `scholaralerts-noreply@google.com` → "Google Scholar"）自动设置来源名称
- **发件人筛选**: `GMAIL_SENDER_FILTER` 作为允许列表，过滤非目标发件人
- **去重**: `_deduplicate()` — 按标题（不区分大小写）跨所有邮件去重
- **功能**: 日期过滤、标记已读（`thread.markAsRead()`，trash 前强制执行）、处理标签（`message.addLabel()`）、失败标记未读（`thread.markAsUnread()`）、处理成功后移入回收站（`thread.trash()`，默认开启）
...
- **方法**:
  ...
  - `enrich_paper(paper) → PaperItem`: **DOI → 标题 → URL** 顺序查找。CrossRef **覆盖**可用字段。额外元数据存入 `extra["crossref"]`；未匹配时写入 `extra["crossref_unmatched"]`（doi/title/url）。
...
- **方法**:
  ...
  - `enrich_paper(paper) → PaperItem`: **DOI → 标题 → URL** 顺序查找。**只补充缺失字段**。额外元数据存入 `extra["openalex"]`；未匹配时写入 `extra["openalex_unmatched"]`（doi/title/url）。
...
- **一级（关键词 OR）**: 快速、低成本的宽筛选。若未传 `--keywords`，会根据 `RESEARCH_PROMPT` 自动生成关键词。
...
# RSS 源配置
PAPER_FEEDDER_MCP_OPML=feeds/RSS_official.opml  # 默认 OPML 文件路径
PAPER_FEEDDER_MCP_USER_AGENT=paper-feedder-mcp/2.0  # 统一 User-Agent
...
GMAIL_SENDER_FILTER=scholaralerts-noreply@google.com,noreply@nature.com  # 发件人筛选列表
...
# OpenAlex 限流（建议配置）
OPENALEX_API_KEY=your-openalex-key  # OpenAlex API 需要 key（免费）
...
## 10. 命令行接口 (`client/cli.py`)

`paper-feedder-mcp` CLI 为所有框架操作提供统一接口。
...
- **`fetch`**: 从数据源收集论文。
  ...
  - `--query`: Gmail 搜索查询。
  - `--limit` / `--since`: 抓取约束（`--since` 默认近 15 天）。
- **`filter`**: 应用过滤管道。
  - `--input` / `--output`: JSON 文件路径。
  - `--keywords`: 关键词过滤（OR 逻辑，一级宽筛选）。
  - `--authors` / `--exclude`: 作者/排除关键词过滤。
  - `--min-date` / `--has-pdf`: 元数据过滤。
  - `--ai` / `--no-ai`: 启用或禁用 LLM 语义过滤（默认启用）。
- **`enrich`**: 通过外部 API 进行元数据补充。
  - `--api` / `--source` / `--provider`: `crossref`、`openalex` 或 `all`（默认）。
  - `--jobs` / `--concurrency` / `--parallel`: 最大并行 API 请求数（默认: 5，通过 `asyncio.Semaphore` 控制）。
  - **执行逻辑**（`--api all` 时）: CrossRef 与 OpenAlex 依次执行（均会尝试补充缺失字段）。每个 client 在 `finally` 中调用 `close()` 释放连接。
- **`export`**: 导出论文到目标格式。
  - `--format`: `json`（默认）或 `zotero`。
  - `--include-metadata` / `--no-metadata`: 是否包含扩展字段（`extra`，默认包含）。
- **`delete`**: 删除 `output/` 目录及常见中间文件（如 `raw.json`、`filtered.json`、`enriched.json` 等）。
...
- `export(papers, filepath="output/papers.json", include_metadata=True) → Dict`（默认 `include_metadata=True`）
- 将 `List[PaperItem]` 导出为 JSON 文件（`model_dump()` 序列化）
- 将 `date` 对象转换为 ISO 格式字符串
- `include_metadata=False` 时移除 `extra` 字段
...
- 通过本地 `zotero-mcp` 仓库导出到 Zotero 库
- 默认路径：`E:\Desktop\SciPapers\zotero-mcp\src`，或设置 `ZOTERO_MCP_PATH`
...
| **配置** | `config/settings.py` | 基于 dotenv 的集中配置 |
| **元数据补充** | `sources/crossref.py`, `sources/openalex.py` | 过滤后通过外部 API 补充元数据 |
| **失败开放** | `filters/ai_filter.py` | AI 过滤出错时返回所有论文 |
...
- **类型注解**: 所有地方。允许使用 `T | None`（Python 3.12）
...
- **不要更改 PaperItem schema** 除非绝对必要 — 使用 `extra` 字典替代
...
- **总计：346 通过，2 跳过（Zotero 可选依赖），0 失败**
...
3. **保留 PaperItem schema**: 使用 `extra` 字典进行来源特定扩展；不要在没有充分理由的情况下添加新的顶层字段。
...
**4. 数据管理**
- 中间 JSON 文件便于调试和重新过滤
- 用完可执行 `paper-feedder-mcp delete` 一键清理输出目录
- 使用有意义的文件名（如 `2026-02-09_battery_papers.json`）
...
# 1. 检查 API key 是否正确
# 2. 检查网络连接
# 3. 查看详细日志：uv run paper-feedder-mcp filter ... 2>&1 | tee log.txt
```
...
在 `ai/keyword_generator.py` 和 `filters/ai_filter.py` 中使用。
